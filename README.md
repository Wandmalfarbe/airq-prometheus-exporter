...
This is a Prometheus exporter for the air-Q, an air quality measuring device.
The air-Q data is exported as Prometheus text metrics (or JSON) and
can be visualized with tools such as Grafana.
...
| virus          | Virus-free Index            | Virenfrei-Index                | %       | double | yes               |
| ch2o_M10       | Formaldehyde                | Formaldehyd                    | µg/m³   | double | yes               |
| mold           | Mold-free Index             | Schimmelfrei-Index             | %       | double | yes               |
...
| pm2_5_SPS30    | PM2.5 Particulate Matter    | PM2.5 Feinstaub                | µg/m³   | double | yes               |
| pm10_SPS30     | PM10 Particulate Matter     | PM10 Feinstaub                 | µg/m³   | double | yes               |
| pm4_SPS30      | PM4 Particulate Matter      | PM4 Feinstaub                  | µg/m³   | double | yes               |
| pm1_SPS30      | PM1 Particulate Matter      | PM1 Feinstaub                  | µg/m³   | double | yes               |
| TypPS_SPS30    | Typical Particle Size       | Typ. Partikelgröße             | µm      | double | no                |
...
| dCO2dt         | Change of CO₂ concentration | Änderung der CO₂ Konzentration | ppb/s   | int    | no                |
| dHdt           | Change of Humidity          | Änderung der Luftfeuchtigkeit  | mg/m3/s | int    | no                |
...
|----------------------------------------|--------------------------------------|---------------|---------------------------------------------------------------------------------------------------|
| `-h` or `--airq-host "<host>"`         | `AIRQ_PROM_EXP_AIRQ_HOST`            |               | IP address or host name of the air-Q e.g. `192.168.178.46` or `31b31_air-q.local`.                |
| `-p` or `--airq-password "<password>"` | `AIRQ_PROM_EXP_AIRQ_PASSWORD`        |               | Password of the air-Q                                                                             |
...
| `-o` or `--export-other-metrics`       | `AIRQ_PROM_EXP_EXPORT_OTHER_METRICS` | false         | Export additional (non-air-Q) metrics e.g., metrics about the prometheus exporter itself.         |
...
```shell
java -jar airq-prometheus-exporter.jar --airq-host "192.168.178.46" --airq-password "myPassword123" --port "8080"
```

One can also set the environment variables and call the application without arguments.

```shell
export AIRQ_PROM_EXP_AIRQ_HOST="192.168.178.46"
export AIRQ_PROM_EXP_AIRQ_PASSWORD="myPassword123"
...
```shell
docker run -d -e export AIRQ_PROM_EXP_AIRQ_HOST="192.168.178.46" AIRQ_PROM_EXP_AIRQ_PASSWORD="myPassword123" AIRQ_PROM_EXP_PORT="8080" -p 8080:8080 --name air-prometheus-exporter air-prometheus-exporter
```
...
1. `http://localhost:<port>/metrics`
    ...
    airq_timestamp{device="abcdefg",} 1.707151608E12
    # HELP airq_uptime Uptime (s)
    # TYPE airq_uptime gauge
    airq_uptime{device="abcdefg",} 4598.0
    # HELP airq_pressure Air Pressure (hPa)
    # TYPE airq_pressure gauge
    airq_pressure{device="abcdefg",} 1006.89
    # HELP airq_co Carbon Monoxide (mg/m³)
    # TYPE airq_co gauge
    airq_co{device="abcdefg",} 0.403
   ...
    ```

2. `http://localhost:<port>/json/data-raw`
...
3. `http://localhost:<port>/json/data-parsed`
...
```shell
rm "airq-prometheus-exporter.tar"
docker build -t airq-prometheus-exporter . --platform linux/arm64/v8
